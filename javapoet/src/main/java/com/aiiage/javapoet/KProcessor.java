...
import com.aiiage.libannotation.KAnnotation;
...
public class KProcessor extends AbstractProcessor {
    ...
    public boolean process(Set<? extends TypeElement> annotations, RoundEnvironment roundEnv) {
        for (TypeElement element : annotations) {//遍历扫描到的注解集合
            if (element.getQualifiedName().toString().equals(KAnnotation.class.getCanonicalName())) {
                // 当前注解是我们自定义的注解，也就是HelloAnnotation时，执行下列代码
                TypeSpec helloWorld = TypeSpec.classBuilder("KAnnotationTest")//声明类名为HelloWorld
                        .addModifiers(Modifier.PUBLIC, Modifier.FINAL)//声明类的修饰符为 public final
                        .addMethod(getMethodSpec("kHhello1", "Hello kotlin"))//为HelloWorld类添加名为hello1的方法，返回值为Hello
                        .build();
    ...
    public Set<String> getSupportedAnnotationTypes() {
        return Collections.singleton(KAnnotation.class.getCanonicalName());
    }
